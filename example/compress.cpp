...
int main(int argc,char *argv[])
{
    ...
    ifs.seekg(0,std::ios::end);//定位到文件末尾
    size_t fsize=ifs.tellg();//获取文件当前位置相对与起始位置的偏移量
    ifs.seekg(0,std::ios::beg);
    std::string body;
    body.resize(fsize);//调整str大小未文件大小
    ifs.read(&body[0],fsize);//读取文件所有的数据到str中
    std::string packed=bundle::pack(bundle::LZIP,body);//以LZIP格式压缩文件数据
    std::ofstream ofs;
    ofs.open(ofilename,std::ios::binary);//打开打压缩包文件
    ofs.write(&packed[0],packed.size());//将压缩后的数据写入压缩包
    ifs.close();
    ofs.close();
    return 0;
}
